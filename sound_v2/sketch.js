...
function keyTyped() {
	if (key === "q") {
		fourSeas.pause() & luvSic.pause() & refEtern.pause() & feather.play();
	}  else if (key === "w") {
		fourSeas.pause() & feather.pause() & refEtern.pause() & luvSic.play();
	} else if (key === "e") {
		fourSeas.pause() & luvSic.pause() & feather.pause() & refEtern.play();
	} else if (key === "r") {
		feather.pause() & luvSic.pause() & refEtern.pause() & fourSeas.play();
	} else if (keyCode === 32) { //space key
		fourSeas.pause() & luvSic.pause() & refEtern.pause() & feather.pause();
	}
	return false;
}